  ...
  useEffect(() => {
    if(activeCategory === 'all'){
      ...
      setFilteredBlogs((prev) => {
        const filteredBlogs = blogs.filter((blog) => blog.bcat.toLowerCase() === activeCategory.toLowerCase())
                ...
                <div key={blog._id} className={classes.blog}>
                  ...
                  <div className={classes.blogData}>
                    <div className={classes.categoryAndMetadata}>
                      <span className={classes.category}>{blog?.bcat}</span>
                      <div className={classes.metadata}>
                        <MdOutlinePreview /> {blog.bviews} views
                      </div>
                      <div className={classes.metadata}>
                        <AiFillLike /> {blog?.blikes?.length} likes
                      </div>
                    </div>
                    <h4>{blog?.btitle}</h4>
                    ...
                    <div className={classes.authorAndCreatedAt}>
                      <span><span>Author:</span> {blog?.uid?.uname}</span>
